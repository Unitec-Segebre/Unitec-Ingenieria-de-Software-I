class Lot < ApplicationRecord
  ...
  def self.chart_values(id)
    case id
      ...
      when 5
        {'vAxis' => 'valorizations.cost_mano', 'vAxisLabel' => 'Costo de Insumo'}
      else
  ...
  #e.g. { name: "VarName", unit_cost_mano: 200, unit_cost_mano: 300, amount: 2, cost_mano: 400 }
  ...
  def getValue(name)
    ...
    unless value.nil?
      hash.merge!({ unit_cost_mano: value.unit_cost_mano, unit_cost_insumo: value.unit_cost_insumo, amount: value.amount, cost_mano: value.cost_mano })
    end
