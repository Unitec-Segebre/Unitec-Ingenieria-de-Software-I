class Lot < ApplicationRecord
  ...
  def self.columns_chart
    [['Cantidad','1'], ['Costo por Mano de Obra','2'], ['Costo por Hectárea por Mano de Obra','3'], ['Costo por Hectárea por Insumo','4'], ['Costo por Insumo','5'], ['Costo por Hectárea','6']]
  end
  ...
  def today_values(category)
    self.variables
      .select("variables.id, variables.name, valorizations.unit_cost_mano, valorizations.amount, valorizations.unit_cost_insumo, valorizations.unit_cost_total")
  ...
  def range_values(category, var_id)
    self.variables
      .where("category": category)
      .where("id": var_id)
      .select("variables.id, variables.name, valorizations.amount, valorizations.unit_cost_mano, valorizations.cost_mano, valorizations.unit_cost_insumo, valorizations.cost_insumo, valorizations.unit_cost_total, valorizations.assigned_at")
  ...
  def getValue(name)
    ...
    hash = { name: var.name, unit_cost_mano: var.unit_cost_mano, unit_cost_insumo: var.unit_cost_insumo, unit_cost_total: var.unit_cost_total, amount: 0, cost_mano: 0, cost_insumo: 0 }
    unless value.nil?
      hash.merge!({ unit_cost_mano: value.unit_cost_mano, unit_cost_insumo: value.unit_cost_insumo, unit_cost_total: value.unit_cost_total, amount: value.amount, cost_mano: value.cost_mano, cost_insumo: value.cost_insumo })
    end
