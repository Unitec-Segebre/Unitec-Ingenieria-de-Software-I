class Lot < ApplicationRecord
  ...
  def getValue(name)
    ...
    hash = { name: var.name, unit_cost_mano: var.unit_cost_mano, amount: 0, subtotal: 0 }
    unless value.nil?
      hash.merge!({ unit_cost_mano: value.unit_cost_mano, amount: value.amount, subtotal: value.subtotal })
    end
